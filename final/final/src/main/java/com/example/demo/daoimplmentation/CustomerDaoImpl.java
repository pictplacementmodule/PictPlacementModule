...
public class CustomerDaoImpl extends JdbcDaoSupport implements CustomerDao {
	...
	public List<AdminPlaced> adminplaced() {
		...
		{
			String sql = "select p.count as pcount,p.id as pid,concat(s.fn,' ',s.ln) as sfn,c.name as cname,p.package_lpa as pack,p.location as loc,p.pl_status as status from placedstudents as p,Studentpersonal_Details as s,industry as c where p.id = s.rollno AND c.id = p.comp_id AND p.pl_status=1";
			List<Map<String, Object>> rows = getJdbcTemplate().queryForList(sql);
			...
			for (Map<String, Object> row : rows) {
				...
				cus.setStatus((Integer) row.get("status"));
				result.add(cus);
			}
		} catch (Exception e) {
...
/*
 * @Override public Customer findCustomerById(long cust_id) { String sql =
 * "SELECT * FROM customer WHERE CUST_ID = ? "; return
 * (Customer)getJdbcTemplate().queryForObject(sql, new Object[]{cust_id}, new
 * RowMapper<Customer>(){
 * 
 * @Override public Customer mapRow(ResultSet rs, int rwNumber) throws
 * SQLException { Customer cust = new Customer();
 * cust.setCustId(rs.getLong("cust_id")); cust.setName(rs.getString("name"));
 * cust.setAge(rs.getInt("age")); return cust; } }); }
 * 
 * @Override public String findNameById(long cust_id) { String sql =
 * "SELECT name FROM customer WHERE cust_id = ?"; return
 * getJdbcTemplate().queryForObject(sql, new Object[]{cust_id}, String.class); }
 */