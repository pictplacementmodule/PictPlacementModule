    ...
    axios.post("/industry/edit", state)
      .then((response) => {
        ...
        if (response.data) {
          ...
          setMessage("Company Profile not edited!");
          setOpen(true);
          setVariant("error");
        }
      })
    ...
    (event) => {
      ...
      switch (name) {
        ...
        case "contactno3":
          ...
          break;
        case "criteria":
          error = (Number(value) > 0 && Number(value) < 10)
            ? ""
            : "Invalid Criteria";
          break;
        case "package_lpa":
          reg = /^[0-9]+(\.[0-9]+)?$/;
          error = (reg.test(value) && Number(value) > 0)
            ? ""
            : "Invalid package_lpa";
          break;
        case "no_of_students":
          error = (Number(value) > 0 && Number(value) > 0)
            ? ""
